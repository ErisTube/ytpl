...
class UTILS {
     ...
     */
    tryParseBetween(body, left, right, addEndCurly = false) {
        try {
    ...
    parseBody(body, options = {}) {
        const json = this.tryParseBetween(body, 'var ytInitialData = ', '};', true) ||
        ...
        const apiKey = this.between(body, 'INNERTUBE_API_KEY":"', '"') ||
            this.between(body, 'innertubeApiKey":"', '"');
        const clientVersion = this.between(body, 'INNERTUBE_CONTEXT_CLIENT_VERSION":"', '"') ||
        ...
        if (options.gl)
            context.client.gl = options.gl;
        if (options.hl)
            context.client.hl = options.hl;
    ...
    async doPost(url, opts, payload) {
        ...
        return (0, undici_1.request)(url, reqOpts).then(r => r.body.json());
    }
    /**
    ...
    checkArgs(listId, options = {}) {
        ...
        if (!this.getPropInsensitive(obj.requestOptions.headers, 'user-agent')) {
            this.setPropInsensitive(obj.requestOptions.headers, 'user-agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) ' +
                'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Safari/537.36');
        }
        const cookie = this.getPropInsensitive(obj.requestOptions.headers, 'cookie');
        if (!cookie) {
            this.setPropInsensitive(obj.requestOptions.headers, 'cookie', CONSENT_COOKIE);
        }
        else if (!cookie.includes('SOCS=')) {
            this.setPropInsensitive(obj.requestOptions.headers, 'cookie', `${cookie}; ${CONSENT_COOKIE}`);
        }
        return obj;
    }
    /**
     ...
     */
    setPropInsensitive(obj, prop, value) {
        const key = this.findPropKeyInsensitive(obj, prop);
        obj[key || prop] = value;
        return key;
    }
}
exports.default = Object.assign(new UTILS(), {});
